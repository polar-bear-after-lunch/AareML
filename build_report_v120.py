...
# ── Build document ─────────────────────────────────────────────────────────
out = "/home/user/workspace/AareML-report.pdf"  # v1.20
...
def toc_row(label, page_hint, style, anchor_name=None):
    """TOC row with clickable left label and right-aligned page number."""
    # ReportLab internal links use <link destination="name"> tag
    link_open  = f'<link destination="{anchor_name}" color="#01696F">' if anchor_name else ''
    link_close = '</link>' if anchor_name else ''
    dot_count  = max(2, 80 - len(label) - len(str(page_hint)))
    dots       = '\u00b7' * (dot_count // 2)
    return Paragraph(
        f'{link_open}{label}{link_close}  <font color="#BAB9B4">{dots}</font>  {page_hint}',
        style
    )
...
story.append(p(
    "Page numbers are approximate — regenerated each build. "
    "All section headings and appendix references are clickable.",
    "caption"
))
