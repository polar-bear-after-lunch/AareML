...
# ── Build document ─────────────────────────────────────────────────────────
out = "/home/user/workspace/AareML-report.pdf"  # v1.18
...
story.append(p(f"Report version: 1.18  ·  Last updated: {_now}", "meta"))
story.append(sp(32))
...
usgs_section_content.append(Paragraph(
    "Mean RMSE of 1.376 mg/L is substantially worse than Swiss zero-shot performance (0.464 mg/L), "
    "as expected given the complete absence of US training data. Nevertheless, the Willamette River "
    "(0.996 mg/L) approaches the LakeBeD-US LSTM reference (1.40 mg/L) \u2014 a Pacific Northwest "
...
story.append(p(
    "<b>A1 \u2014 Architecture:</b> LSTM and GRU achieve nearly identical DO RMSE (0.290 vs 0.293 mg/L), "
    "confirming that the Seq2Seq inductive bias is more important than the gating mechanism. "
    "The 0.003 mg/L gap is well within bootstrap CI overlap and not significant. "
    "We retain LSTM as it is marginally better and more studied in hydrology (Kratzert et al. 2019). "
    "<b>A2 \u2014 Teacher Forcing:</b> Removing teacher forcing entirely (TF=0.0) degrades RMSE "
    "by 0.012 mg/L \u2014 the largest single degradation in the ablation. "
    "Full teacher forcing (TF=1.0) also degrades performance (+0.005 mg/L) because the decoder never "
    "learns to handle its own errors during training. "
    "The Optuna-selected TF=0.5 with linear decay is the optimal balance. "
    "<b>A3 \u2014 Loss Function:</b> MSE-only and NSE+MSE achieve virtually identical RMSE (0.001 mg/L gap). "
    "However, NSE+MSE achieves higher KGE (0.920 vs 0.916), indicating better preservation of "
    "distributional properties \u2014 consistent with the theoretical motivation for the combined loss. "
    "<b>A4 \u2014 Lookback Window:</b> Each additional 7-day extension improves RMSE: "
    "7d (0.300) \u2192 14d (0.294) \u2192 21d (0.290). "
    "The consistent improvement suggests the model benefits from a full three-week context window, "
    "likely capturing weekly temperature cycles and catchment residence times."
))
story.append(PageBreak())
...
# ══════════════════════════════════════════════════════════════════════════
story.append(h2("5.9  Seasonal Analysis"))
story.append(p(
    "Table 7 shows model performance at gauge 2473 stratified by season (2017\u20132020 test set). "
    "Seasons are assigned by the window start date: DJF (Dec\u2013Feb), MAM (Mar\u2013May), "
    "JJA (Jun\u2013Aug), SON (Sep\u2013Nov). Day-1 and day-14 RMSE show how forecast uncertainty "
    "accumulates across the 14-day horizon."
))

# Seasonal table
sea_data = [
    [Paragraph(h, S["table_header"]) for h in
     ["Season", "Mean DO (mg/L)", "RMSE", "NSE", "RMSE day 1", "RMSE day 14", "Degradation"]],
    [p("DJF (Winter)","table_cell"), p("12.50","table_cell_c"),
     p("0.326","table_cell_c"), p("0.099","table_cell_c"),
     p("0.149","table_cell_c"), p("0.386","table_cell_c"), p("2.6\u00d7","table_cell_c")],
    [p("MAM (Spring)","table_cell"), p("11.50","table_cell_c"),
     p("0.337","table_cell_c"), p("0.510","table_cell_c"),
     p("0.168","table_cell_c"), p("0.399","table_cell_c"), p("2.4\u00d7","table_cell_c")],
    [p("JJA (Summer)","table_cell"), p("10.28","table_cell_c"),
     p("<b>0.252</b>","table_cell_c"), p("<b>0.652</b>","table_cell_c"),
     p("0.135","table_cell_c"), p("0.293","table_cell_c"), p("2.2\u00d7","table_cell_c")],
    [p("SON (Autumn)","table_cell"), p("11.27","table_cell_c"),
     p("0.304","table_cell_c"), p("0.787","table_cell_c"),
     p("0.124","table_cell_c"), p("0.386","table_cell_c"), p("3.1\u00d7","table_cell_c")],
]
sea_tbl = Table(sea_data, colWidths=[3.0*cm, 3.0*cm, 2.0*cm, 2.0*cm, 2.3*cm, 2.3*cm, 2.4*cm])
sea_tbl.setStyle(TABLE_STYLE)
story.append(sea_tbl)
story.append(sp(4))
story.append(p(
    "Table 7: Seasonal performance at gauge 2473. "
    "Best RMSE and NSE shown in bold. Day-14/Day-1 RMSE ratio shown as 'Degradation'. "
    "DJF NSE is low despite moderate RMSE because winter DO variance is low "
    "(NSE = 1 \u2212 SS\u209b\u209e\u209b / SS\u209c\u2092\u209c, denominator is small).",
    "caption"
))

story.append(p(
    "Several findings stand out. "
    "First, JJA (summer) achieves the <b>lowest absolute RMSE</b> (0.252 mg/L) \u2014 counterintuitively, "
    "the model performs best precisely when fish are most at risk from low DO. "
    "This is because summer DO is dominated by the strong temperature signal (temp[t\u22121] dominates SHAP), "
    "and temperature is highly predictable at 14-day horizons. "
    "Second, <b>DJF has the lowest NSE</b> (0.099) despite not being the worst in RMSE \u2014 "
    "this is a mathematical artefact: winter DO sits near 12\u201313 mg/L with low variance, "
    "so the NSE denominator (SS\u209c\u2092\u209c) is small and small absolute errors produce low NSE. "
    "Third, <b>horizon degradation is sharpest in autumn</b> (SON: 3.1\u00d7 from day 1 to day 14), "
    "likely because autumn transitions (leaf fall, catchment flushing) create "
    "non-stationary dynamics that are harder to project forward. "
    "These seasonal patterns are consistent across all 12 transfer gauges "
    "(see notebook 13 and results/seasonal_results.csv)."
))
story.append(PageBreak())
