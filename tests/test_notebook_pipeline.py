"""
AareML notebook pipeline tests — organized into three test groups.
"""
...
class TestNotebooksBasics:
    """Checks that notebooks have been run and key outputs exist."""
    ...
    def test_ar_baseline_results(self):
    ...
    def test_cv_transfer_results(self):
        ...
        mean_rmse_val = df["rmse_do"].mean()
        assert 0.3 < mean_rmse_val < 1.0, f"Mean RMSE {mean_rmse_val:.3f} outside expected range"

    def test_ridge_transfer_results(self):
    ...
    def test_ea_lstm_results_updated(self):
    ...
    def test_temp_multisite_results(self):
    ...
    def test_nb18_cascaded_results(self):
...
class TestModelsAndMetrics:
    """Checks model results are within expected numeric ranges."""
    ...
    # ── Per-gauge Ridge extraction correctness ────────────────────────────
    ...
    def test_per_gauge_rmse_non_negative(self):
        ...
        N, L, F = X.shape
        _, H, T = y.shape
        ridge_scaler = StandardScaler().fit(X.reshape(N, L * F))

        DO_IDX = list(TARGETS).index(TARGETS[0])
        ridge_models = {}
        for h in range(H):
            ridge_models[h] = {}
            for j in range(T):
...
class TestSignificanceAndBaselines:
    """Checks statistical tests and baseline comparisons."""
    ...
    def test_significance_tests_exist(self):
